...
#add dropout
class LeNetVar3(nn.Module):
    def __init__(self, num_classes=10):
        super(LeNetVar3, self).__init__()
        self.conv1 = nn.Conv2d(1, 6, kernel_size=5)
        self.pool1 = nn.AvgPool2d(kernel_size=2)
        self.conv2 = nn.Conv2d(6, 16, kernel_size=5)
        self.pool2 = nn.AvgPool2d(kernel_size=2)
        
        self.dropout_conv = nn.Dropout(p=0.25)  # Dropout after conv/pool
        self.dropout_fc   = nn.Dropout(p=0.5)   # Dropout in fully connected layers
...
model_var3 = LeNetVar3(num_classes=10).to(device)
criterion = nn.CrossEntropyLoss()
optimizer = torch.optim.Adam(model_var3.parameters(), lr=0.001)
...
for epoch in range(1, epochs + 1):
    train_one_epoch(model_var3, device, train_loader, optimizer, criterion, epoch)

total_training_time = time.time() - total_start_time  # Compute total training time
print(f"Total training time: {total_training_time:.2f} seconds.")

accuracy = test(model_var3, device, test_loader, criterion)
