...
class LeNetVar1(nn.Module):
    def __init__(self, num_classes=10):
        super(LeNetVar1, self).__init__()
        # Convolutional layers
        self.conv1 = nn.Conv2d(1, 16, kernel_size=5, padding=2)   # input: 1x28x28 -> output: 16x28x28
        self.pool1 = nn.AvgPool2d(kernel_size=2, stride=2)         # output: 16x14x14
        self.conv2 = nn.Conv2d(16, 32, kernel_size=5)               # output: 32x10x10
...
model_var1 = LeNetVar1(num_classes=10).to(device)
criterion = nn.CrossEntropyLoss()
optimizer = torch.optim.Adam(model_var1.parameters(), lr=0.001)
...
for epoch in range(1, epochs + 1):
    train_one_epoch(model_var1, device, train_loader, optimizer, criterion, epoch)

total_training_time = time.time() - total_start_time  # Compute total training time
print(f"Total training time: {total_training_time:.2f} seconds.")

accuracy = test(model_var1, device, test_loader, criterion)
